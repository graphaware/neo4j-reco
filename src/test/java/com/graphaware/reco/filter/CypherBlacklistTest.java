...
/**
 *  Test for {@link CypherBlacklist}.
 */
public class CypherBlacklistTest extends DatabaseIntegrationTest {
    ...
    public void shouldBlacklistMatchingNodes() {
        ...
        CypherBlacklist cypherBlacklist = new CypherBlacklist(getDatabase()) {
            @Override
        ...
        try (Transaction tx = getDatabase().beginTx()) {
            Set<Node> blacklist = cypherBlacklist.getBlacklist(getSingle(getDatabase().findNodesByLabelAndProperty(PERSON, "name", "Michal")));

            assertEquals(1, blacklist.size());
            assertEquals("GraphAware", blacklist.iterator().next().getProperty("name"));

            blacklist = cypherBlacklist.getBlacklist(getSingle(getDatabase().findNodesByLabelAndProperty(PERSON, "name", "Emil")));
